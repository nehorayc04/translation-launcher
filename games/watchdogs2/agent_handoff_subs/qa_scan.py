"""BUG-FIX pass — scan every already-translated SUBTITLE line for STRUCTURAL defects
and REMOVE the bad ones from hebrew.json, so get_batch.py re-serves them and you
re-translate them correctly in the loop. Run ONCE at the start (and any time to
re-verify). Does NOT translate.

Catches: empty · foreign script · niqqud · lost/added tokens · model-refusal leak
· length blow-up (rambling) · untranslated English leak (with a name/code passthrough
so a proper noun kept in Latin is NOT flagged).
...
for k, he in heb.items():
    ...
    if not he or not he.strip():
        ...
        r = "refusal_leak"
    elif len(en) >= 8 and len(he) > 2.6 * len(en) + 50:
        r = "length_anomaly"
