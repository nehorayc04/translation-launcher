"""I/O helper — validate + merge your SUBTITLE translations into hebrew.json.
Does NOT translate.
...
# NOTE: bracket tokens are matched case-INSENSITIVELY here — subtitles use lowercase
# cue tokens ([laughs] [beat] [hacker]) AND mixed-case ([PlayerName]) AND uppercase
# ([HACK] [CSS_END]). The literal line-break "\n" is NOT validated (kept by instruction).
TOKEN = re.compile(
    r'\[[A-Za-z0-9_]+\]'           # [HACK] [hacker] [laughs] [PlayerName] [CSS_END] ...
    r'|\{[^}]*\}'                  # {threshold} {VALUE}
    r'|%[0-9.]*[diufslxeDIUFSLXE]+|%%'   # %d %s %ls %0.2f %u %i %%
    r'|&#?[A-Za-z0-9]+;')          # &#xA; &amp; &gt; &lt; &nbsp;
