#!/bin/bash
# One-shot pull of every stream's out.json -> host bank. Registered as a Windows Scheduled Task
# (every 3 min) so pulling survives session teardowns (unlike a bash run_in_background loop).
KEY=~/.ssh/id_ed25519
...
LOCK=/c/tmp/pull_once.lock
# skip if a pull is already in flight (lock < 150s old)
if [ -f "$LOCK" ]; then
...
pull() {  # name host port user remotepath
  local dest="$BANK/retrans_agent_$1/retrans_corrections.json"; local tmp="$dest.pull.tmp"
  rm -f "$tmp"
  ...
  if [ -s "$tmp" ] && "$PY" -c "import json,sys;json.load(open(sys.argv[1],encoding='utf-8'))" "$tmp" 2>/dev/null; then
    mv -f "$tmp" "$dest"; echo "$(date +%H:%M:%S) $1 pulled ($(wc -c <"$dest") B)" >> "$LOG"
  else rm -f "$tmp"; echo "$(date +%H:%M:%S) $1 unreachable" >> "$LOG"; fi
}
