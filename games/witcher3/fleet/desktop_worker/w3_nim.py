"""Witcher 3 NIM translator — DESKTOP 6th stream (mop-up).

Identical to fleet/w3_nim.py EXCEPT the todo set also skips anything ANY stream already banked
(the global ../hebrew.json), re-read each outer loop — so this 6th machine never double-does the
5 remote streams' work; it dynamically translates only what's still missing. corpus.json = the
FULL W3 corpus {key: en}; out.json = this stream's own output (pulled into banks/out_desktop.json).
"""
...
GBANK = os.path.join(HERE, "..", "hebrew.json")   # global merged bank (skip-set)
...
def _load_bank():
    if os.path.exists(GBANK):
        try: return json.load(open(GBANK, encoding="utf-8"))
...
def main():
    ...
    print(f"keys={len(_KEYS)} | model={MODEL} | corpus={len(corpus)} (DESKTOP mop-up, skips global bank)")

    while True:
        out = json.load(open(OUT, encoding="utf-8")) if os.path.exists(OUT) else {}
        skip = _load_bank()                                   # anything ANY stream already banked
        ...
        if not todo:
            print(f"✅ nothing left to mop up ({len(out)} done here). Sleeping 120s."); time.sleep(120); continue
        batches = make_batches(todo)
        print(f"remaining {len(todo)} (global-skip {len(skip)}) | done here {len(out)} | {len(batches)} batches x{NW}")
        t0 = time.time(); prod = 0
