#!/usr/bin/env python3
r"""
aor_loc.py — AC Origins (scimitar v28) LocalizationPackage codec.

Chain:  forge entry -> CFD chain (Oodle, aor_cfd) -> Anvil object
...
patch forge encrypted them; Origins's do not.
...
There are TWO length fields that must BOTH be re-derived on any content change:
  ...
  • count     at marker+4      = len(payload)
    ...
    python aor_loc.py list  <forge>
    python aor_loc.py dump  <forge> <name|#index> [out.json]
    python aor_loc.py stats <forge>
"""
import argparse
import json
import os
...
def iter_packages(fg, od=None, names_only=False):
    ...
    od = od or aor_cfd.oodle()
    for e in fg.entries:
        try:
            blob = fg.read(e)
            if aor_cfd.peek_class(blob, od) != LOC_CLASS:
                continue
            parts = aor_cfd.decode_resource(blob, od)
...
def find(fg, key, od=None):
    ...
    if key.startswith("#"):
        ...
        return Package(e, aor_cfd.decode_resource(fg.read(e), od))
    best = None
