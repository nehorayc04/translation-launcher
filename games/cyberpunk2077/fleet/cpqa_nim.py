...
def _acquire_singleton():
    try:
        import msvcrt
        fh = open(os.path.join(HERE, f"cpqa{_SUF}.lock"), "w")
        try:
...
def main():
    ...
    # 🔑 A per-provider corpus (corpus_<prov>.json) OVERRIDES the md5%3 split — see the same block
    # in w3qa_nim.py. Measured 2026-07-26: nim's third was 100% done (all 3 nim workers had exited)
    # while groq+sambanova still held all 1,414 remaining lines, i.e. 6 of 9 streams idle.
    ...
    print(f"keys={len(_KEYS)} | model={MODEL} | corpus={len(corpus)} "
          f"({'per-provider file' if per_prov else 'md5%3'}) (CP2077 FULL line-by-line QA)")
