# Spider-Man 2 (New-Era-2 REVIEW) fleet watchdog -- runs every 5 min, hidden, no popups.
#
...
#   3. a DEAD or MISSING worker: fewer than 3 sm2ne2_nim on a machine.
#   4. a DISABLED launch task -- Sm2MP was found Disabled on the desktop, which silently
...
$LogPath   = 'C:\tmp\sm2ne2_watchdog.log'
...
$ZOMBIE    = 'sm2ne2_nim_ZOMBIE_PLACEHOLDER_never_matches'
...
# 🔴 ONLY THE MACHINES THIS GAME OWNS (games/spiderman2/fleet/machines.json). The desktop is
# finishing Skyrim; relaunching Sm2MP there would put two fleets on one machine's key set.
$machines = @(
  @{ n='laptop';  port=22;   user='Nehoray_Cohen'; host='10.0.0.49'; dir='C:/Users/Nehoray_Cohen/Projects/sm2_worker' }
  @{ n='vm4';     port=2225; user='vboxuser';      host='10.0.0.49'; dir='C:/sm2w' }
  @{ n='vm5';     port=2226; user='vboxuser';      host='10.0.0.49'; dir='C:/sm2w' }
  @{ n='vm';      port=2222; user='vboxuser';      host='127.0.0.1'; dir='C:/sm2w'; vbox='Win11 - 1'  }
  @{ n='vm2';     port=2223; user='vboxuser';      host='127.0.0.1'; dir='C:/sm2w'; vbox='Win11-VM-2' }
  @{ n='vm3';     port=2224; user='vboxuser';      host='127.0.0.1'; dir='C:/sm2w'; vbox='Win11-VM-3' }
)
...
`$w=@(Get-CimInstance Win32_Process -Filter "name='python.exe'" | Where-Object { `$_.CommandLine -match 'sm2ne2_nim' }).Count
`$t=0
Get-ScheduledTask | Where-Object { `$_.TaskName -match '^Sm2' -and `$_.State -eq 'Disabled' } | ForEach-Object { Enable-ScheduledTask -TaskName `$_.TaskName | Out-Null; `$t++ }
if (`$w -lt 3) { schtasks /run /tn Sm2MP | Out-Null }
...
foreach ($m in $machines) {
  ...
  if ($out) {
    ...
    if ($s -match 'w=(\d+)' -and [int]$Matches[1] -lt 3) { Say "$name only $($Matches[1])/3 workers -> Sm2MP triggered" }
    continue
  }
...
if (-not (Get-ScheduledTask -TaskName 'Sm2FleetPull' | Where-Object { $_.State -ne 'Disabled' })) {
  Enable-ScheduledTask -TaskName 'Sm2FleetPull' | Out-Null; Say 'Sm2FleetPull re-enabled'
}
$push = @(Get-CimInstance Win32_Process -Filter "name='python.exe'" |
          Where-Object { $_.CommandLine -match 'sm2qa_progress' })
if ($push.Count -eq 0) {
  Invoke-CimMethod -ClassName Win32_Process -MethodName Create -Arguments @{
    CommandLine    = '"C:\Users\Nehoray_Cohen\AppData\Local\Programs\Python\Python313\python.exe" -u sm2qa_progress.py'
