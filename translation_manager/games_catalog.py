...
class CatalogGame:
    ...
    # Paid-mod price in agorot. Mirrors the live `games.price_cents` so the DRM
    # gate stays CLOSED even on the offline bundled-catalog fallback (a missing
    # price would resolve to 0 = "free for everyone" - the "Build G" hole class).
    price_cents: int = 0
...
SHOWCASE: list[CatalogGame] = [
    ...
    CatalogGame(
        ...
        price_cents=5300,   # paid mod - keep the DRM gate closed offline too
    ),
    CatalogGame(
        "corsair-cove", "Corsair Cove", "מפרץ השודדים",
        "1.0.0-beta.1", "default", "available",
        "הרפתקת פיראטים בעולם פתוח",
        "בנו צי, גלו איים אבודים והשתלטו על הים הקאריבי - עם תרגום עברי מלא לממשק ולעלילה.",
    ),
    CatalogGame(
    ...
    CatalogGame(
        ...
        "1.0.0-beta.1", "hogwarts", "available",
...
PLANNED: list[CatalogGame] = [
    ...
    CatalogGame("tlou2", "The Last of Us Part II", "דה לאסט אוף אס · חלק II",
        "-", "default", "coming-soon",
        "מסע נקמה אכזרי ושובר לב",
        "המשיכו את סיפורם של אלי ואבי במסע נקמה טורד מנוחה בעולם שאחרי המגפה."),
    ...
    CatalogGame("anno1800", "Anno 1800", "אנו 1800",
        "1.0.0-beta.2", "default", "available",
        ...
        price_cents=5300),   # paid mod - keep the DRM gate closed offline too
    # ── Action / platformer ──
    # id matches the future website/Supabase games row so detection + the live
    # catalog line up on the same id when this ships.
    CatalogGame("ratchet-rift-apart", "Ratchet & Clank: Rift Apart", "רצ'ט אנד קלאנק: ריפט אפארט",
        "-", "default", "planned",
        "קפצו בין ממדים עם רצ'ט וריבט",
        "הרפתקת אקשן-פלטפורמר צבעונית שקופצת בין ממדים במהירות הבזק, עכשיו ב-PC. "
        "עזרו לרצ'ט ולריבט לעצור את הדוקטור נפריוס ולהציל את המולטי-יקום."),
    # ── Story / survival ──
    CatalogGame("plague-tale-innocence", "A Plague Tale: Innocence", "א פלייג טייל: אינוסנס",
        "-", "default", "planned",
        "הישרדות באפלת המוות השחור",
        "צרפת, 1349. אמיסיה ואחיה הקטן הוגו נמלטים מהאינקוויזיציה ומנחשולי עכברושים "
        "קטלניים בימי המוות השחור. הרפתקת הישרדות-חמקנות אפלה ומרגשת."),
    CatalogGame("plague-tale-requiem", "A Plague Tale: Requiem", "א פלייג טייל: רקוויאם",
        "-", "default", "planned",
        "המסע האפל של אמיסיה והוגו נמשך",
        "ההמשך ל-Innocence: אמיסיה והוגו נמלטים דרומה אל חופי הים התיכון בחיפוש אחר "
        "מרפא, בעוד קללת העכברושים רודפת אחריהם. הרפתקת אקשן-הישרדות קולנועית ועוצמתית."),
    # ── Interactive drama / horror ──
    # id matches the website/Supabase games row ("until-dawn") so the launcher's
    # detection + the live catalog line up on the same id when this ships.
    CatalogGame("until-dawn", "Until Dawn", "עד עלות השחר",
        "-", "default", "coming-soon",
        "שמונה חברים, לילה אחד, גורל שנקבע בבחירות שלכם",
        "רימייק 2024 לאימה האינטראקטיבית הקלאסית - כל החלטה קובעת מי שורד עד עלות השחר "
        "בהר בלאקווד."),
    # ── Open-world action (Ubisoft Dunia) ──
    # id matches the website/Supabase games row ("farcry6").
    CatalogGame("farcry6", "Far Cry 6", "פאר קריי 6",
        "-", "default", "coming-soon",
        "מהפכה בגן עדן טרופי - הפילו את הרודן אנטון קסטיו",
        "פאר קריי 6 מציב אתכם כדני רוחאס, לוחם גרילה במדינת יארה הבדיונית בהשראת קובה. "
        "הצטרפו למהפכה נגד הדיקטטור העריץ אנטון קסטיו ובנו דייגו, בעולם פתוח ענק ופראי."),
    # id matches the website/Supabase games row ("farcry5"). Dunia engine (FAT2 v10).
    CatalogGame("farcry5", "Far Cry 5", "פאר קריי 5",
        "-", "default", "planned",
        "כת יום-הדין השתלטה על מונטנה - ואתם השריף שנשאר לבד",
        "פאר קריי 5 מציב אתכם כסגן שריף במחוז הופ שבמונטנה, שנשלח לעצור את ג'וזף סיד - "
        "מטיף כריזמטי שהפך כת יום-דין חמושה לשליטת האזור. המעצר משתבש, המחוז נסגר, "
        "ואתם נותרים לבנות התנגדות מבית לבית ולהפיל אחד-אחד את בני משפחת סיד."),
]
