# -*- coding: utf-8 -*-
"""A Plague Tale: Requiem — push live fleet translation progress to the hub home page.
...
games.id ("plague-tale-requiem"). Cloud NIM → no local GPU.
...
SENT_HIST = os.path.join(HERE, "pt_progress_sent.json")     # [[ts, done_sentences], ...] for the rate

GAME_ID = "plague-tale-requiem"                       # MUST match Supabase games.id
...
def _sent_map():
    ...
    Non-translatable MARKERS (onom/mono/EMPTY/URLs/native language-names — they keep the shipping
    Arabic, never get Hebrew) are EXCLUDED so the % reflects the real translatable work."""
    try:
...
def main():
    ...
    while True:
        try:
            ...
            bank = {}
        done_sent = _done_sentences(bank.keys(), sent)
