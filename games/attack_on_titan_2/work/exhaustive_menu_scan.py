"""Definitive exhaustive scan: every entry, every candidate archive, both flat
DataTables and group tables (one level of nesting), exact-match only against
the 8 title-screen menu words. Prints every hit with surrounding context so we
can judge, once and for all, whether a self-contained menu list exists
anywhere in the game's LINKDATA text archives."""
...
CANDIDATES = [
    ...
    LD / "REGION" / "LINKDATA_REGION_JP.BIN",
    LD / "REGION" / "LINKDATA_REGION_AS.BIN",
...
def all_tables(content):
    """Yield (label, strs_list) for the top-level table, or each nested table
    if this is a group table."""
...
for path in CANDIDATES:
    ...
    print(f"  -> {file_hits} exact hits in this archive")
    print()

print(f"TOTAL exact hits across all candidates: {total_hits}")
