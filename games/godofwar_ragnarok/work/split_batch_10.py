...
def main():
    ...
    untrans = [k for k in sorted(ar.keys(), key=int) if k not in heb and k in en]
    
    batch_keys = untrans[:500]
    print(f"Total untranslated: {len(untrans)}. Selected first {len(batch_keys)} for Batch 10.")
    print(f"Batch 10 key range: {batch_keys[0]} to {batch_keys[-1]}")
    
    part_size = 125
    for i in range(4):
        part_keys = batch_keys[i*part_size : (i+1)*part_size]
        part_data = [[k, en[k]] for k in part_keys]
        part_file = f"batch_10_part{i+1}.json"
