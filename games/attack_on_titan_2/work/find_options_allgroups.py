...
from aot2_linkdata import decompress_blocks, parse_datatable, read_cstring, MAGIC, is_datatable
...
WANT = {"Control Assistance", "Extra-wall Map Speed", "Skip Journey Events",
        "Preferred Input Method", "Default Network Settings",
        "Game 1", "Game 2", "Controls", "Camera", "Audio", "Graphics 1",
        "Graphics 2", "Keyboard and Mouse", "Offline"}
...
for gi, off in enumerate(offs):
    ...
    for si, s in enumerate(strs):
        if s is not None and s.strip() in WANT:
            print(f"group {gi} [{si}] = {s!r}  (of {len(strs)} strings)")
