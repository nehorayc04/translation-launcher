#!/usr/bin/env python3
r"""
aor_font.py — inject the Hebrew block into AC Origins's `FontFile` resources.
...
skipped by default (they are huge and never draw Latin/Hebrew UI).
    ...
    python aor_font.py <forge> list
    python aor_font.py <forge> inject <out_dir> [--donor PATH] [--all]
"""
import argparse
import io
import os
...
class FontRes:
    ...
    def __init__(self, entry, blob, od):
        self.entry = entry
        self.parts = aor_cfd.decode_resource(blob, od)
    ...
    def cfd_parts(self, ttf: bytes):
        """[(data, cinfo)] ready for aor_cfd.encode_resource."""
    ...
    @property
    def codec(self):
        return next((c for _, _, c in self.parts if c), aor_cfd.OODLE_KRAKEN)
...
def font_entries(fg, od):
    for e in fg.entries:
        try:
            blob = fg.read(e)
            if aor_cfd.peek_class(blob, od) != FONT_CLASS:
                continue
        except Exception:
            continue
        yield e, blob
...
def main():
    ...
    for e, blob in font_entries(fg, od):
        ...
        res = aor_cfd.encode_resource(fr.cfd_parts(new_ttf),
