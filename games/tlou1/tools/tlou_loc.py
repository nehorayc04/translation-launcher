...
def encode(orig_data, overrides):
    ...
    ⚠️ Why surgical (learned in-game on TLOU2R 2026-07-07, same ND codec):
    a full rebuild (reorder + re-dedup the whole blob) is read back CORRECTLY
    by our own decoder (offset table stays consistent) but the GAME ENGINE
    reads UNCHANGED strings WRONG (e.g. "Downtown" -> "Tntoe") — it relies on
    the original blob layout / offsets (sid-lookup references them). So
    `encode(orig, {})` MUST be byte-identical to `orig`; an override must not
    move a single unchanged byte. Grows freely (offsets are u64).
