#!/usr/bin/env python3
r"""
aor_scan.py — locate AC Origins resources by CONTENT (ScimitarClass hash).
...
Cheap by construction: `aor_cfd.peek_class` inflates ONLY the first block of the
object CFD, so a multi-GB forge is scanned without decompressing it.

    python aor_scan.py hist  <forge> [--limit N]          # class histogram
    python aor_scan.py find  <forge> <class_hash|name>    # entries of that class
    python aor_scan.py sweep <class_hash|name> [forges…]  # all forges in the game dir
"""
import argparse
import os
import sys
...
GAME = os.environ.get("ACO_GAME", r"F:\Games\Assassin's Creed Origins")
