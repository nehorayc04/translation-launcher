#!/bin/bash
# Gender-review pull+merge for vm3 + vm4 + vm5 + laptop (vm3 localhost; vm4/vm5/laptop via Tailscale).
# Same SAFE merge as pull_gender.sh: a proposal is accepted only if it is a pure gender inflection of
# the current line (identical non-Hebrew scaffold + each changed word lev<=3). Writes gender_overrides.json.
KEY=~/.ssh/id_ed25519
...
pull(){ # name host port user remotepath
  ...
  timeout 45 scp -i "$KEY" -P "$3" -o StrictHostKeyChecking=no -o BatchMode=yes -o ConnectTimeout=20 "$4@$2:$5" "$tmp" 2>/dev/null
...
# 5 streams: vm/vm2/desktop local + vm4/laptop via Tailscale (laptop back on-net). vm3/vm5 stopped —
# their gbanks (out_vm3/out_vm5.json) stay in the merge (reviewed work kept).
pull vm     127.0.0.1     2222 vboxuser      "C:/ptwg/out.json"
pull vm2    127.0.0.1     2223 vboxuser      "C:/ptwg/out.json"
pull vm4    100.116.78.88 2225 vboxuser      "C:/ptwg/out.json"
pull laptop 100.116.78.88 22   Nehoray_Cohen "C:/Users/Nehoray_Cohen/ptwg/out.json"
# desktop is LOCAL
...
def scaffold(s): return re.sub(r'[֐-׿]','',s)
...
def safe(old,new):
    ...
    if scaffold(new)!=scaffold(old): return False
    ow,nw=words(old),words(new)
    if len(ow)!=len(nw): return False
    for a,b in zip(ow,nw):
        if a!=b and lev(a,b)>3: return False
    return True
