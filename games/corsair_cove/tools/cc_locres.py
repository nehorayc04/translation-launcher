...
def _write_unreal_string(s):
    ...
    # to what the engine's own cooker emits.
    if s == "":
        return _write_i32(0)
    s = s + "\x00"
...
def save(parsed, path):
    ...
    string_table = []  # ordered unique values (first-appearance order == the game's own order)
    string_index = {}  # value -> index
    ref_counts = []    # per-index reference count (the game stores the REAL count)
    total_entries = 0
