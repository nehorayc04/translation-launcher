...
def replace_file_data(node, path, new_bytes, compress=True):
    ...
    f._dirty = True                      # marks it for in-place append
    return True
...
# re-pack (serialize_open_rpf) reorders + tight-packs EVERY file (dropped ~800 MB
# of original padding from update.rpf) → the engine failed with ERR_GEN_ZLIB_2.
# --------------------------------------------------------------------------- #
def serialize_inplace(original_buf, root, base=0):
    """Return original_buf with ONLY the dirty files appended at the end (512-aligned)
    and their TOC entries repointed. Entry count / names / all unchanged files and their
    offsets are byte-identical to the original. base = the RPF header offset (0 for a
    top-level archive)."""
    out = bytearray(original_buf)
    files = []
    ...
    for n in files:
        ...
        offblocks = off // 512
        out += n.data
        out += b"\x00" * ((-len(out)) % 512)               # pad to next 512 block
        packed = (n._name_off & 0xFFFF) | ((n.csize & 0xFFFFFF) << 16) | \
                 ((offblocks & 0x7FFFFF) << 40) | ((n._res & 1) << 63)
        eoff = base + 16 + n._idx * 16
        struct.pack_into("<Q", out, eoff, packed)
        struct.pack_into("<II", out, eoff + 8, n.usize or len(n.data), n.flags)
    return bytes(out)
