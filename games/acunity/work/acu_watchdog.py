...
multi-day gemma-4-31b-it run. It is the single thing to launch.
  ...
  1. LOCAL MODEL — verifies gemma-4-31b-it stays loaded in LM Studio. The model
     is SHARED with a second parallel translation run (Watch Dogs 2), so reloads
     mirror the live --context-length / --parallel (currently --parallel 2) and
     never downgrade it. If it drops, reloads it automatically (clears the known
...
# LM serving config — PRESERVED across watchdog reloads so a recovery never
# downgrades a user-chosen setting. The model is shared with a second parallel
# translation run (Watch Dogs 2), so it must stay at --parallel 2 (NOT 1).
# reload_lm() mirrors the live `lms ps` values when it can; these are the
# fallback when the model has already dropped out of memory. Env-overridable.
LM_CONTEXT  = os.environ.get("SM2_LM_CONTEXT", "2048")
LM_PARALLEL = os.environ.get("SM2_LM_PARALLEL", "2")
...
def capture_lm_config():
    """Read the loaded model's live CONTEXT + PARALLEL from `lms ps` and
    remember them, so a reload re-applies the SAME settings the user chose
    instead of forcing defaults (this is what keeps --parallel 2 sticky for
    the shared SM2 + WD2 run). Falls back to the last-known / env values when
    the model has already dropped out of `lms ps`. Returns (context, parallel)."""
    try:
        out = subprocess.run([LMS, "ps"], capture_output=True, text=True,
                             timeout=30).stdout or ""
...
def reload_lm():
    ...
    ctx, par = capture_lm_config()   # mirror the LIVE setting — never downgrade --parallel
