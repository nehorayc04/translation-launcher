...
from aot2_linkdata import decompress_blocks, parse_datatable, read_cstring

GAME_ROOT = Path(r"F:\Games\Attack on Titan 2")
path = GAME_ROOT / "LINKDATA" / "REGION" / "LINKDATA_REGION_EDEN_EU.BIN"
...
for lo, hi in [(695, 725), (800, 825)]:
    print(f"--- indices {lo}-{hi} ---")
    for i in range(lo, min(hi, len(strs))):
        print(f"  [{i}] {strs[i]!r}")
    print()
