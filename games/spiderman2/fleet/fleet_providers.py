...
def _build_ssl_ctx():
    ...
    # Found 2026-08-02 on vm2/vm3 (this game's fleet): identical certifi
    # bundle (234354 B) on every machine but ssl.create_default_context()
    # loaded 23/21/21 CAs off Windows' own store - pinning certifi's bundle
    # explicitly makes every machine trust the exact same, complete root set
    # regardless of what Windows has or hasn't auto-fetched. Same fix applied
    # to games/rdr2/fleet/fleet_providers.py 2026-08-02.
    try:
        import certifi
...
class Fleet:
    ...
    def _pick(self):
        ...
        time.sleep(max(0.0, _COOL.get(p, 0) - now) + 0.3)
        return p, b, m
    ...
    def complete(self, sysmsg, usermsg, retries=4, timeout=120, max_tokens=2500):
        ...
        # at ONE real attempt + one short-cooldown retry, not four long ones. (Same fix
        # applied to games/rdr2/fleet/fleet_providers.py 2026-08-02 - this is a separate
        # per-game copy of the same file and had the identical stale logic.)
        last = None
        attempts = 2 if len(self.avail) == 1 else max(retries, len(self.avail))
        for _ in range(attempts):
            p, b, m = self._pick()
            try:
                return _one(b, m, self.keys[p], sysmsg, usermsg, timeout, max_tokens)
