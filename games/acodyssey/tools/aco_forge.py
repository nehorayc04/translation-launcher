#!/usr/bin/env python3
r"""
aco_forge.py — Assassin's Creed Odyssey (AnvilNext 2.0) scimitar-**v28** .forge reader.
Pure Python, READ-ONLY, no deps.

🔑 ENGINE-FAMILY REUSE: v28 is BYTE-IDENTICAL in layout to Mirage's v29
(`games/acmirage/tools/mirage_forge.py`) — the Mirage reader parses every Odyssey
forge unchanged (validated: DataPC 4,533/4,533 · DataPC_extra 18,098/18,098 ·
TitleScreen 287/287, 0 contiguity errors). This file is the Odyssey-local copy so
the game owns its own toolchain, with the version guard widened to {28, 29}.

  Header:
    0x00  char[8] "scimitar" + u8 0x00
    0x09  u32   version                (== 28 for Odyssey / Origins-era)
...
Usage:
    python aco_forge.py <forge> info
    python aco_forge.py <forge> list [--limit N]
    python aco_forge.py <forge> extract <id|#index> <out.bin>
"""
import argparse
import os
import struct
