# -*- coding: utf-8 -*-
"""Witcher 3 line-by-line QA worker (NIM fleet) — reviews EVERY Hebrew line against its English
source AND the game's own gendered languages (Arabic + Russian + Polish + Spanish + Italian).
Checks, per line: ADDRESSEE gender/number, SPEAKER gender, unnatural PHRASING, wrong register,
...
SAFETY (never degrade a good line — monotonic, can only FIX):
  • GENDER: a gender/number flip is accepted ONLY when the multi-language evidence baked into the
    item (ag=addressee m/f/pl, num='pl' true-plural, formal=formal-SINGULAR trap) confirms the NEW
    Hebrew form; a genderless line with no oracle is never flipped.
  • FORMAL-YOU TRAP: ru вы / it voi / es usted addressed to ONE person is FORMAL SINGULAR, NOT
    plural (plural is taken ONLY from Arabic أنتم) -> reject any Hebrew->אתם edit when formal.
...
def _load_out():
    """Load our bank, surviving a file the last hard power-cycle left unflushed.

    🔴 The VM watchdog recovers a hung guest with `poweroff`, and NTFS can leave an
    in-flight write allocated at full size but filled with NULs — the file LOOKS like
    549 KB of work and decodes to nothing. Crashing here put the worker in a permanent
    restart loop (2026-07-22, vm/nim: down for hours behind a tidy log). Move the bad
    file aside and start clean; the review is monotonic, so the slice is merely re-done.
    """
    ...
    try:
        ...
        try:
            ...
            pass
        print(f"!! {os.path.basename(OUT)} unreadable ({e}) — moved to "
              f"{os.path.basename(bad)}, starting from empty", flush=True)
        return {}
...
def _acquire_singleton():
    try:
        import msvcrt
        fh = open(os.path.join(HERE, f"w3qa{_SUF}.lock"), "w")
        try:
            msvcrt.locking(fh.fileno(), msvcrt.LK_NBLCK, 1)
        except OSError:
            print("w3qa_nim already running on this stream -> exit", flush=True)
            sys.exit(0)
        return fh
    except SystemExit:
        raise
...
def gender_facts(v):
    ...
    when a non-Arabic language marks plural but Arabic does NOT (a polite-SINGULAR you -> Hebrew
    must stay אתה/את, never אתם). Arabic decides alone; else >=2 non-Arabic agree on m/f."""
...
S1 = ("You are a senior Hebrew localizer for The Witcher 3: Wild Hunt (dark medieval fantasy — "
      "bestiary, alchemy, journals, gwent, letters). Each item has the English SOURCE, the current "
      "HEBREW line, and the game's PROFESSIONAL translations in ARABIC (ar), RUSSIAN (ru), POLISH "
      ...
      "Also fix: UNNATURAL/literal phrasing → natural literary period Hebrew; clear MISTRANSLATION "
      "vs English; leftover FOREIGN/English/Arabic words that should be Hebrew.\n"
      "HARD RULES: keep the SAME meaning; keep every tag/placeholder VERBATIM (<br> <i></i> <font..> "
      "{..} %d %s &ent;); keep proper NAMES (Geralt=גרלט, Ciri=סירי, Yennefer=יניפר, Novigrad=נוביגרד) "
      "and NUMBERS; NO niqqud; store LOGICAL Hebrew (no letter-reversal, no RTL marks). If already "
      "correct, return it UNCHANGED. Output JSON {id:{\"he\":<hebrew>,\"iss\":<ok|gender|phrasing|"
      "error|foreign>}} — same ids, nothing else.")
...
def main():
    ...
    # 🔑 A per-provider corpus (corpus_<prov>.json) OVERRIDES the md5%3 split. The hash third is
    # fine on a fresh corpus but goes badly skewed the moment one provider is rate-limited and
    # falls behind: measured 2026-07-26, groq held 5,160 of the 7,567 remaining lines while the
    # other two thirds were finished and their workers had EXITED. A reslice
    # (universal/fleet_reslice_equal.py) then hands every stream an exactly equal share.
    ...
    print(f"keys={len(_KEYS)} | corpus={len(corpus)} ({'per-provider file' if per_prov else 'md5%3'})"
          f" | provider={_PROV or 'round-robin'} (Witcher 3 QA)")
