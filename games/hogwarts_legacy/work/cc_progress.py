# -*- coding: utf-8 -*-
"""Push Hogwarts Legacy COMMUNITY-COMPUTE progress to the hub dashboard.

Same design as games/ratchet_rift_apart/work/cc_progress.py (the volunteer-phone fleet's
next game after R&C finished 17,624/17,624): DONE/OPEN/CLAIMED/STREAMS come from the
community control plane (Supabase `cc_stats(p_secret, p_game)` RPC), GAME-FILTERED so this
game's numbers are never mixed with any other game sharing the same cc_lines queue.

Counted in LINES. 60 s loop -> /api/admin/progress.
...
GAME_ID = "hogwarts"                  # MUST match the Supabase games row id (dashboard joins on it)
...
def cc_stats():
    """{open, claimed, done, workers} for THIS game only (game-filtered — the queue is shared)."""
...
def main():
    ...
    print(f"Hogwarts community pusher (gameId={GAME_ID}) every {INTERVAL}s -> {API}"
          + (" [ONCE]" if once else ""))
    while True:
        try:
            one(token)
