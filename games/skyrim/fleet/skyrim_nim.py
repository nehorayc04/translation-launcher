"""Skyrim translator worker — New-Era, six-language panel.
...
Run: python skyrim_nim.py <groq|sambanova|nim>   (keys in keys.json next to this file)
"""
...
S1 = ("You are a senior Hebrew localizer for The Elder Scrolls V: Skyrim Anniversary Edition. "
...
MAX_TFAILS = 3
SKIP = os.path.join(HERE, f"cc_skip{_SUF}.json")
...
def main():
    ...
    print(f"Skyrim New-Era-2 worker | slice {len(corpus)} lines | already done {len(out)} | "
    ...
    while True:
        ...
        over = [k for k, v in todo if len(_en(v)) > MAX_EN_CHARS]
        if over:
        ...
        tokonly = [k for k, v in todo
                   if (_en(v).strip() and not STRUCT.sub("", _en(v)).strip())
                   or (_en(v) and not _en(v).strip())]
        if tokonly:
