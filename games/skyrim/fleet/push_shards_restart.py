...
machines = [
    ...
    {"name": "vm", "dir": "C:/skyrimw", "ssh": "127.0.0.1", "port": 2222, "user": "vboxuser", "vm": True},
    {"name": "vm2", "dir": "C:/skyrimw", "ssh": "127.0.0.1", "port": 2223, "user": "vboxuser", "vm": True},
    {"name": "vm3", "dir": "C:/skyrimw", "ssh": "127.0.0.1", "port": 2224, "user": "vboxuser", "vm": True},
]
...
CODE = ["skyrim_nim.py", "fleet_providers.py", "brain_glossary.json"]
...
for m in machines:
    ...
    script = (ipv6 +
              "Get-CimInstance Win32_Process | Where-Object { $_.Name -eq 'python.exe' -and $_.CommandLine -match 'skyrim_nim' } | "
              "ForEach-Object { try { Stop-Process -Id $_.ProcessId -Force } catch {} }; "
              f"Remove-Item '{mdir}/worker_*.lock' -ErrorAction SilentlyContinue; Start-Sleep 2; "
              "schtasks /run /tn SkyrimMP | Out-Null; Write-Output RESTARTED")
    try:
        if is_local:
