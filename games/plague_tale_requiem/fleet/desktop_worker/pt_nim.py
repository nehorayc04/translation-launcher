"""Standalone A Plague Tale: Requiem NIM translator — SINGLE-Hebrew {key: he} (NOT dual-gender).

Same fleet mechanism as the Witcher 3 worker (per-source-IP NIM quota, resumable via out.json, a
disjoint corpus.json slice), adapted for Plague Tale Requiem:
  * game = grim historical-fiction, 1349 plague-ravaged southern France (Inquisition, rat swarms,
    alchemy); characters Amicia, Hugo, Lucas, Beatrice, Vaudin.
  * tokens = the pipe '|' (an in-value LINE BREAK — NOT \n) and {STR_...} runtime button tokens,
    both preserved VERBATIM (same count/positions).
  * store LOGICAL Hebrew here; the RTL transform (pt_rtl.to_stored — reverse LTR islands, keep the
    Hebrew logical) happens at BUILD time in games/plague_tale_requiem/work/, NEVER in the worker.

Put the NVIDIA key in key.txt (nvapi-... line) or NVIDIA_API_KEY. Run: python pt_nim.py
...
NW = 1         # single key per stream -> serial (NIM parallelism comes from the separate keys, not threads)
CORPUS = os.path.join(HERE, "corpus.json")
OUT = os.path.join(HERE, "out.json")

FOREIGN = re.compile(r'[؀-ۿ぀-ヿ一-鿿가-힣Ѐ-ӿ]'); NIQ = re.compile(r'[֑-ֽֿׁׂ]'); HEB = re.compile(r'[֐-׿]')
# PT tokens: {STR_...} runtime button tokens + the pipe '|' line-break. Preserve their multiset.
STRUCT = re.compile(r'\{[^}]*\}|\||%%|%[#0-9.*\-+]*[a-zA-Z]+'); LOWER = re.compile(r'[a-z]{2,}')
_NAMEWORD = re.compile(r"^[A-Z0-9][\w.\-'/]*$"); _CTRL = "".join(chr(c) for c in range(0x20))

S1 = ("You are a senior Hebrew localizer for A Plague Tale: Requiem (a grim historical-fiction game "
      "set in 1349, plague-ravaged southern France — the Inquisition, swarms of rats, alchemy). "
      ...
      "(a line spoken to a woman gets את/feminine verbs, to a man gets אתה/masculine). Do NOT translate "
      "FROM the Arabic and do NOT copy Arabic words — 'en' is the meaning, 'ar' is only the gender hint. "
      "Keep every token VERBATIM from the English: the pipe '|' (a LINE BREAK — same count and "
      "positions) and {STR_...} button tokens. No niqqud. Proper names (Amicia, Hugo, Lucas, Beatrice, "
      "Vaudin, Sophia, places) stay as their accepted Hebrew transliteration; brand/code tokens stay "
      "Latin. Output JSON {id: hebrew} only, same ids as the input.")


def _en(v):
    """corpus value → English (supports both the {'en','ar'} shape and a bare string)."""
...
def make_batches(todo):
    """token-budget batches: a long line gets its own call; short lines pack together."""
    batches, cur, ct = [], [], 0
    for k, v in todo:
        t = _tok(v)
...
def main():
    ...
    print(f"keys={len(_KEYS)} | model={MODEL} | corpus={len(corpus)} (Plague Tale Requiem, gender-aware: en+ar)")

    while True:
        out = json.load(open(OUT, encoding="utf-8")) if os.path.exists(OUT) else {}
        todo = [(k, corpus[k]) for k in corpus if k not in out]
        todo.sort(key=lambda kv: _tok(kv[1]))   # SHORTS FIRST (small=fast+NIM-reliable), long lines last
        if not todo:
