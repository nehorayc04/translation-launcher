...
REM Dev build counter — increments every build. Shown EVERYWHERE as the full
REM launcher version v1.0.0-dev.<N> (launcher UI + website + release row).
REM Persisted in the gitignored _dev_build.txt next to this script.
set "DEV_BUILD=0"
if exist "_dev_build.txt" set /p DEV_BUILD=<"_dev_build.txt"
set /a DEV_BUILD+=1
> "_dev_build.txt" echo %DEV_BUILD%
...
echo Baked _build_info.py — BUILD_ID=%BUILD_ID% DEV_BUILD=%DEV_BUILD%
