...
namespace RDR2_RPF_Tool.Core
{
	...
	public class Tfit2CbcCipher : ICipher
	{
		...
		private static ulong TFIT2_DecryptRoundBlock(ulong[] input1, ulong[] masks, uint xorr, ulong[] lookup)
		{
			...
			upper |= upper << 7;
			upper |= upper >> 14;
			// (the decompiler emitted a native-int cast here; a plain int index is equivalent)
			return lookup[(int)((lower & 255UL) ^ (upper & 3840UL) ^ (ulong)xorr)];
		}
		...
		public byte[] Decode(byte[] input, int? start = null, int? lenght = null)
		{
			...
			for (int i = StartPosition; i < StartPosition + Size; i += 16)
			{
				...
				for (int j = 0; j < 16; j++)
				{
					// FIXED: the decompiled source wrote to input[j] instead of input[i + j],
					// which corrupts every block after the first.
