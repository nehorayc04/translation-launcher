#!/usr/bin/env python3
r"""
aco_cfd.py — decode + encode AC Odyssey CompressedFileData (CFD) blobs.

🔑 ENGINE-FAMILY REUSE: Odyssey (scimitar v28) uses the SAME CFD container as
...
the object CFD. decode_resource()/encode_resource() handle the list.
    ...
    python aco_cfd.py roundtrip <forge> <index>   # decode -> re-encode -> verify
    python aco_cfd.py codec     <forge> <index>   # report the shipped Oodle codec
"""
import os
import struct
import sys
...
GAME_DEFAULT = r"F:\Games\Assassin's Creed Odyssey"
