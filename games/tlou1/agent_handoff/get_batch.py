...
`batch_<slot>.json` = { key: {"cat":.., "en": <english>, "he": ""} }. The agent fills
every "he" with LOGICAL Hebrew (natural reading order - NOT reversed), preserving every
token, then runs merge_batch.py. Repeat until it prints "All done for this slot!".

Batches are served in VISIBILITY order ([[community-pool-by-category]]): the interface
finishes before story subtitles, which finish before ambient barks - so a partial ship
always covers what players actually see.
"""
import os
import sys
import json
...
def main():
    ...
    cats = _load("categories.json", {})        # {key: Hebrew category} -> visibility order
    done = done_keys()
    mine = sorted((k for k in k2en
                   if (int(k, 16) % nslots) == slot and k not in done),
                  key=lambda k: (CAT_ORDER.get(cats.get(k, ""), 9), k))
    batch = {}
    for k in mine[:BATCH]:
        entry = {}
        if cats.get(k):
            entry["cat"] = cats[k]              # register hint: ממשק=terse, כתוביות=spoken
        entry["en"] = k2en[k]
        g = gsrc.get(k, {})
