"""SM2 Hebrew Subtitle/Dialogue Translator
Translates untranslated SM2 entries to Hebrew using LM Studio (gemma-4-31b-it).
Outputs: subtitles_he.json (entries with <ts> tags) and dialogue_he.json (others).

Usage:
    python sm2_translate.py                # full run
    python sm2_translate.py --dry-run      # scan only, no requests
    python sm2_translate.py --status       # show checkpoint progress
"""
...
def validate(orig_en, translated):
    ...
    if not re.search(r'[א-ת]', translated):
        ...
        # strip placeholders AND html entities/tags (&lt; &nbsp; <br> …) so a
        # markup-only string ("&lt; %s", "&nbsp;<br>") reads as empty → accepted,
        # not mistaken for prose because "lt"/"nbsp" looks like a word.
        text_only = re.sub(r'\[[A-Z_]+\]|\{[^}]+\}|%[\w%]+|&[a-zA-Z#0-9]+;|<[^>]+>',
                           '', text_only).strip()
        if text_only:
            ...
            # a single token with no spaces that is camelCase, carries a digit,
            # or is a long concatenation = a username/handle/identifier (e.g.
            # "purplepowah", "lilMamasPancakes", "granitedoggo") — untranslatable,
            # legitimately stays Latin (else it loops the queue forever).
            is_handle = (" " not in text_only and
                         bool(re.search(r'[a-z][A-Z]', text_only)
                              or re.search(r'\d', text_only)
                              or len(text_only) >= 11))
            if not (is_namey or no_real_word or is_handle):
...
def park_failures(keys):
    ...
    strike; at 3 it joins sm2_translate_skip.json so the queue stops looping on
    it forever (un-translatable handles, 2-bit-quant hallucinations) and the
    build's Arabic/English fallback covers it. Returns the keys parked this call."""
    if not keys:
        return []
    strikes_f = os.path.join(HERE, "sm2_translate_strikes.json")
    skip_f    = os.path.join(HERE, "sm2_translate_skip.json")
    try:
        strikes = json.load(open(strikes_f, encoding="utf-8")) if os.path.exists(strikes_f) else {}
    except Exception:
        strikes = {}
    try:
        skip = set(json.load(open(skip_f, encoding="utf-8"))) if os.path.exists(skip_f) else set()
    except Exception:
        skip = set()
    parked = []
    for k in keys:
    ...
    _atomic_dump(strikes, strikes_f)
    if parked:
...
def main():
    ...
    for bi, b in enumerate(batches):
        ...
        # strike + park anything that failed every attempt this pass so the
        # queue can't loop on it forever (the cause of the done=12603 stall).
