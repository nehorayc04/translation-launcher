# -*- coding: utf-8 -*-
"""Push the Assassin's Creed II translation progress live to the hub dashboard (gameId=ac2),
in PARALLEL with the CP2077-QA and Witcher-3-QA pushers.
...
TOTAL = corpus_full.json. 60 s loop -> /api/admin/progress.
"""
...
_STRUCT = re.compile(r'<[^>]+>|\{[^}]*\}|' + _BR_TOKEN + r'|%%|%[#0-9.*\-+]*[a-zA-Z]+')
...
CORPUS = os.path.join(HERE, "corpus_full.json")
BANK = os.path.join(HERE, "hebrew.json")
HIST = os.path.join(HERE, "ac2_progress_hist.json")

GAME_ID = "ac2"                    # MUST match the Supabase games row id (the dashboard joins on it)
